...
function validateForm(form, validators)
{
	...
	if(!pass)
	{
		...
		if(pageErrorElement)
		{
			pageErrorElement.innerHTML = Framework.messages.thisPageContainsErrors;
	...
	for(var ruleIndex in rules)
	{
		...
		if(!validateValue(value, rule))
		{
			...
			if(!field.hasError)
			{
				...
				fieldValidationErrors.push(new hmrc.portal.FieldValidationError(field.id, errorMessage));
			}
