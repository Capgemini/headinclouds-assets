// Library of functions that need to be available for every page, packaged into a single Framework object.
var Framework = new function()
{
	...
	this.Utility = new function()
	{
		...
		this.addClassName = function(htmlElement, className)
		{
			...
			var newClassName = htmlElement.className.replace(getClassNameRegExp(className), ' ');
		...
		*/
		this.addEventHandler = function(htmlElement, eventName, eventHandler)
		{
			...
			if(!htmlElement[eventCollection])
			{
				...
				htmlElement[eventName] = function(){return fireEvents(this, eventName);};
	...
	this.DisplayConditions = new function()
	{
		...
		function evaluate(eventElement)
		{
			for(var i = 0; i < conditions.length; i++)
			{
				...
				condition.element.style.display = display ? '' : 'none';
		...
		function evaluateExpression(expression)
		{
			...
			expression = expression.replace(/\((.*)\)/g, function($0, $1){return evaluateExpression($1);});

			// evaluate &&
			expression = expression.replace(/(.+?)&&(.+)/g, function($0, $1, $2){return evaluateExpression($1) && evaluateExpression($2);});
	...
	this.FormValidation = new function()
	{
		...
		{
			var functionName = formName.replace(/(.)(.*)/g, function(t,a,b){return a.toUpperCase() + b;});
		...
		this.inject = function()
		{
			...
			if(persistentForm)
			{
				...
				for(var j = 0; j < linksLength; j++)
				{
					...
					if(links[j].href &&
						!Framework.Utility.hasClassName(links[j], 'popup-window') &&
						!Framework.Utility.hasClassName(links[j], 'new-window'))
					{
	...
	this.DeviceProfiling = new function()
	{
		...
		function profile()
		{
			if(typeof dp_addAttributesToPage !== 'undefined')
			{
				...
				window.setTimeout(function(){saveCookie();form.submit();}, 4000);
			}
			else
			{
		...
		function dpSettingsLoaded()
		{
			...
			for(var i = 0; i < formsLength; i++)
			{
				...
				{
					includeDp = false;
					Framework.Utility.addEventHandler(forms[i], 'onsubmit', function(){saveCookie();});
					break;
				}
			}
        ...
        (document.cookie = "test").indexOf.call(document.cookie, "test") > -1));
						...
						{
							//local storage device id isnt valid
							noValidDeviceId();
						}
					}
					else
        ...
        (document.cookie = "test").indexOf.call(document.cookie, "test") > -1));
							...
							else
							{
								if(validateDevice(Framework.Utility.cookies[cookieName]))
								{
									//local storage isnt valid, session storage is
									localStorage.setItem(cookieName, Framework.Utility.cookies[cookieName]);
								}
								else
								{
			...
			//if dp hasnt been carried out and device profiling is required
	...
	this.LinkBackLinks = new function()
	{
		...
		function removeLinks()
		{
			if(typeof Framework.Utility.cookies['du'] === 'undefined')
			{
				...
				var buttonLogoutToDigital = document.getElementById('buttonLogOutTodigital');
				
				if(buttonLogoutToDigital)
				{
